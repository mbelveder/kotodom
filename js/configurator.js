...
/* ---------- Момо говорит ---------- */
/* Баббл остался ровно один — тот, что объясняет неудачу: модулю не нашлось
   места. Остальные реплики (комментарий к каждому поставленному модулю, про
   поворот, форму лаза, скидку, удаление, перенос) сняты: они появлялись на
   каждое действие и превращали подсказку в фон, за которым ошибку уже никто
   не читал. Зовут их отовсюду, поэтому say() не удалён, а замолчал — вызовы
   остались на местах и вернуть болтовню можно одной строкой. */
function say(){ /* молчит намеренно, см. комментарий выше */ }
KD.say = say;

let sayTimer = null;
function warn(text, dur){
  /* пока интро-гид на экране, баббл не показываем: он ушёл бы под затемнение
     (KD.guideShown задаёт app.js) */
  if (KD.guideShown && KD.guideShown()) return;
  /* баббла может не быть в разметке — тогда молча выходим: warn() зовут из
     обработчиков действий, и падение здесь утащило бы за собой всю обработку,
     а не только подсказку */
  ...
  sayTimer = setTimeout(() => momoSay.classList.remove("show"), dur || 5000);
}
KD.warn = warn;
...
function startDrag(e, type, chip){
  ...
  if (!valid.length){
    /* warn, а не say: «некуда поставить» — единственное, ради чего баббл остался */
    warn(NO_SLOT_HINTS[type] || pick(SAY.noSlot));
    chip.animate(
...
/* сборка произвольной конфигурации (пресеты, предложения Момо) с проверкой правил:
   ставим снизу вверх, невалидные ячейки пропускаем — и отчитываемся о них
   бабблом в конце. doneSay (реплика об удачной сборке) больше не показывается:
   баббл остался только для неудач, — но параметр сохранён, его передаёт чат.
   buildGen отменяет «хвост» отложенных постановок, если пользователь успел
   очистить сцену, отменить ход или запустить другую сборку — раньше два
   быстрых клика по планам смешивали их */
  ...
  entries.forEach(([i, t], step) => {
    setTimeout(() => {
      ...
      if (step === entries.length - 1){
        /* warn: это тоже «не нашлось места», только сразу для нескольких
           модулей. Молчать здесь нельзя — иначе из сборки без объяснений
           пропадает часть модулей. Реплика об успешной сборке (doneSay) —
           наоборот, снята вместе с остальной болтовнёй */
        if (skipped.length){
          const names = [...new Set(skipped)].map(x => (MODULES[x] && MODULES[x].name) || x).join(", ");
          warn(`Не поместилось: ${names} — не хватило опоры или места. Передвиньте модули вручную, чтобы освободить место.`, 6500);
        }
      }
    }, step * 160);
  });
